...
\end{center}	
 	...
 	\item Modifica categoria;\\Analogamente si può anche modificare una determinata categoria: più precisamente si può modificare la sua visibilità, le sue 3 card e la sua galleria. Queste tre funzioni sono state divise, per evitare relazioni strette tra loro: può essere necessario dover modificare solo la visibilità o solo la galleria o solo le card, quindi non sarebbe stato conveniente creare un'unica pagina di modifica. In caso di modifica della galleria, è necessario reinserire tutte le immagini con il loro rispettivo alt.
 	
	\item Elimina categoria;\\Una categoria, qualora non fosse più necessaria, la si può eliminare cliccando il pulsante "Elimina". A cascata si elimineranno nel database le immagini delle card e della galleria, i rispettivi paragrafi e le immagini salvate nel server.
\end{itemize}
\begin{figure}[htbp]
\centering
