...
\begin{itemize}
	\item Aggiungi categoria;\\La parte più corposa di questa sezione è rappresentata dall'aggiunta di una categoria: l'amministratore prima deve inserire un titolo e la visibilità (ovvero se nel sito cliente la categoria si vede oppure no), poi inserisce le 3 card principali con i loro rispettivi paragrafi, e infine aggiunge quante immagini vuole per la galleria. 
\begin{center}
...
\end{center}
\begin{center}
\includegraphics[width = 0.3 \textwidth]{../latex/images/pt1.png}\hspace{1em}\includegraphics[width = 0.4 \textwidth]{../latex/images/pt3.png}
\end{center}	
	
 	\item Modifica categoria;\\Analogamente si può anche modificare una determinata categoria: più precisamente si può modificare la sua visibilità, le sue 3 card e la sua galleria. Queste tre funzioni sono state divise, per evitare relazioni strette tra loro: può essere necessario dover modificare solo la visibilità o solo la galleria o solo le card, quindi non sarebbe stato conveniente creare un'unica pagina di modifica.
 	
	\item Elimina categoria;\\Una categoria, qualora non fosse più necessaria, la si può eliminare, e basta cliccare nel pulsante "Elimina". A cascata si elimineranno nel database le immagini delle card e della galleria e i rispettivi paragrafi.
\end{itemize}
\begin{figure}[htbp]
\centering
\includegraphics[width=35mm]{../latex/images/modifica.png}
\caption{Esempio gestione di una categoria già inserita con i pulsanti di modifica visibilità, modifica delle 3 card, modifica galleria e di eliminazione categoria.}
\end{figure}
...
Grazie alla classe \texttt{InputControls} con il linguaggio PHP vengono anche fatti controlli sull'input sui form accessibili a tutti, ovvero il form per le richieste presente nella pagina Contatti e il form per l'autenticazione per accedere alla parte admin.
\end{itemize}
